...
from_name = 'Robot'
to_name = 'Controller'
async def generate_offer(video_track, keyboard_values):
    msg_dict = {
        'from':from_name,
        'to':to_name,
    ...
    offer_dict = await robot_rtc_helper.get_offer(video_track, keyboard_values)
    if(my_uid != -44): 
...
async def handle_message(websocket, message, video_track, keyboard_values):
    global my_uid
    ...
    if('request' in data):
        if(data['request'] == 'offer'):
            offer = await generate_offer(video_track, keyboard_values)
...
async def handle(video_track, keyboard_values):
    async with websockets.connect(server) as websocket:
        try:
            ...
            while True:
                ...
                await handle_message(websocket, message, video_track, keyboard_values)
